#' dge_DESeq
#' @description differential gene expression analysis for raw count expressions using DESeq2 algorithm
...
#'
#' @return \code{list()}, contains expressions, method, design, contrasts, test, and statistics of DESeq2 test.
#' @export
#'
#' @examples
#' \dontrun{
...
dge_DESeq<-function(expressions,sample_frequency_threshold=0.5,clinic_info,ID_col,group_col,covariate_col,block_col,contrasts){
  ...
  keep <- rowSums(counts(dds)) >= ncol(expressions)*2
  keep<-keep & (rowSums(counts(dds)<=1)<(sample_frequency_threshold*ncol(counts(dds))))
  dds <- dds[keep,]
  dds <- DESeq(dds)
  expressions<-tryCatch({assay(vst(dds,blind = F))},error=function(e){assay(vst(dds))})
  ...
  for(contrast in contrasts){
    statistics=as.data.frame(results(dds,contrast=contrast.matrix[,contrast],cooksCutoff = F))[,-1]
  ...
  if(length(contrasts)>=2){
    F_statistics<-as.data.frame(results(dds,cooksCutoff = F))
