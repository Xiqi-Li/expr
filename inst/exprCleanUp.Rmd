...
t1=knitr::kable(SumInfo,caption = "Data loading summary")%>%kableExtra::kable_styling(position = "center",font_size = "200%")
...
rm(list = setdiff(ls(),c(lsf.str(), grep("clean|protein_coding_gene_info|0",ls(),value = T))))
...
unsupervised_results=unsupervised_analysis(unsupervised_data,labels = labels,
...
if(batch_effect_exist){
  ...
  labels=as.factor(clean_RNA_sample_info$batch)
  unsupervised_resultss[["limma"]]=unsupervised_analysis(uns_limma_rbe_log2_protein_expressions,labels=labels,cols = setNames(cols4all::c4a("dark24",nlevels(labels)),levels(labels)))
  #combat
  ...
  unsupervised_resultss[["combat"]]=unsupervised_analysis(uns_combat_rbe_log2_protein_expressions,labels=labels,cols = setNames(cols4all::c4a("dark24",nlevels(labels)),levels(labels)))
