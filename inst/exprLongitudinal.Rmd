...
rm(log2_clean_protein_expressions)
...
DGE=DGE.DT=DGE.plots=volcanoPlots=gseaRess=gseaRes.sig=gseaPlots=list()
```
...
img.gsea=lapply(gseaPlots, function(x) {
  htmltools::plotTag(
  {x},
  sprintf("%s GSEA: significant pathway", names(x)), width = 400, height = 500, pixelratio=3)
})

img.DGE=lapply(volcanoPlots, function(x) {
  htmltools::plotTag(
  {x},
  sprintf("%s DGE: volcano plot", names(x)), width = 400, height = 500, pixelratio=3)
})
...
load(system.file("extdata/pathway.nano_hallmarkv2022.1.RData",package = "expr"))
load(system.file("extdata/protein_coding_ensemble2symbol.RData",package = "expr"))
paramStr=comps_limma=DGE=DGE.plots=DGE.DT=volcanoPlots=gseaRes=gseaRes.sig=gseaPlots=list()
